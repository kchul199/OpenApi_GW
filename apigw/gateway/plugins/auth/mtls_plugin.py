...
class MTLSEnforcerPlugin(BasePlugin):
    ...
    async def __call__(self, request: Request, ctx: GatewayContext, next: NextFunc) -> Response:
        ...
        if not client_cert:
            logger.warning(
                "mTLS Enforcer: Missing client certificate header.",
        ...
        if self._allowed_subjects:
            ...
            if not is_allowed:
                # Log only a safe-length prefix to avoid dumping raw cert data in logs
                logger.warning(
                    f"mTLS: Certificate subject not in allowed list: {client_cert[:40]}..."
                    if len(client_cert) > 40
                    else f"mTLS: Certificate subject not in allowed list: {client_cert}"
                )
                return Response(
